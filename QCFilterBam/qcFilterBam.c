...
int passesFilters(char *row[MAXROW], int wc) {
    ...
    for(i=11;i<wc;i++) {
        ...
        if(!strcmp(tag,"CB")){strcpy(cb, val);found_cbub++;}
        if(!strcmp(tag,"UB")){strcpy(ub, val);found_cbub++;}
    }
...
//fprintf(stderr, "%d ins=%d %d %d %d %c %c %ld %ld\n", Q, insertSize, mismatchesN, gapOpensN, bestHitsN, qstr, mstr, qst, mst);
	...
	//(Q>=minQual) //&&((qstr != mstr)&&(((qstr == '+')&&(qst <= mst))||((mstr == '+')&&(mst <= qst))))){
        ...
        if((found_cbub==2)&&(Q>=minQual)&&(mismatchesN<=maxMismatch)&&(gapOpensN<=maxGapOpen)&&(bestHitsN<=maxBestHit))
            return 1;
    }
...
//fprintf(stderr, "%ld %s\n", ln, row[0]);
		...
		char strnd = (bitFlag & 16) == 0 ? '+' : '-';
                ...
                printf("%s\t%ld\t%ld\t%s\t%s\t%s%s\n",row[2]+chr_offset,coord,coord+getLenFromCigar(row[5])-1,row[5],row[9],cb,ub);
            }
        }
    }
    //lineFileClose(&lf);
...
int main(int argc, char *argv[]) {
    ...
    cb = (char *) calloc(MAXSTR,sizeof(char));
    ub = (char *) calloc(MAXSTR,sizeof(char));
