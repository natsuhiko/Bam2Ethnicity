...
double getRsq(int* dip1, int* dip2, long N){
	...
	dN = (double)N;
        int i;
        for(i=0; i<N; i++){
		if(dip2[i*2]>0 && dip2[i*2+1]>0){
			m2+=2.0; v2+=4.0;
		}else if(dip2[i*2]>0 || dip2[i*2+1]>0){
			m2++; v2++;
		}
		if(dip1[i*2]>0 && dip1[i*2+1]>0){
			m1+=2.0; v1+=4.0;
			if(dip2[i*2]>0 && dip2[i*2+1]>0){
				cv+=4.0;
			}else if(dip2[i*2]>0 || dip2[i*2+1]>0){
		...
		}else if(dip1[i*2]>0 || dip1[i*2+1]>0){
                	m1++; v1++;
			if(dip2[i*2]>0 && dip2[i*2+1]>0){
				cv+=2.0;
			}else if(dip2[i*2]>0 || dip2[i*2+1]>0){
		...
		for(i=i0; i<ret; i++){
			if(buf[i]=='\t' || buf[i]=='\n'){
				...
				if(ncol==0){
					...
					if(genType==FORMAT_GL && existFlag(formatID, nfield, FORMAT_PP)>0){
						gen[ncol-9] = gl[(ncol-9)*3+2]*2.0 + gl[(ncol-9)*3+1];
					}else if(genType==FORMAT_GL && existFlag(formatID, nfield, FORMAT_GL)>0){
